...
namespace
{
	...
	auto get_indices_for_delete_by_query(rsComm_t& comm, const std::string& _object_name, const bool recurs)
		-> std::set<std::string>
	{
		...
		while (!up_node.empty()) {
			if (fsvr::is_collection(comm, up_node)) {
				get_indices_from_collection_AVUs(up_node.string());
			}
			up_node = up_node.parent_path();
			if (0 == up_node.compare(node.root_collection())) {
				break;
			}
		}
		...
		if (recurs) {
			...
			for (; iter != iter_end; ++iter) {
				if (fsvr::is_collection(comm, *iter)) {
					get_indices_from_collection_AVUs(iter->path().string());
				}
			}
		}
	...
	std::tuple<int, std::string> get_index_and_resource(const dataObjInp_t* _inp)
	{
		int l1_idx{};
		...
		if (nullptr == obj_info) {
			THROW(SYS_INVALID_INPUT_PARAM, "no object found");
		}

		std::string resource_name;
		irods::error err =
			irods::get_resource_property<std::string>(obj_info->rescId, irods::RESOURCE_NAME, resource_name);
		if (!err.ok()) {
...
#define NULL_PTR_GUARD(x) ((x) == nullptr ? "" : (x))
	...
	void apply_indexing_policy(const std::string& _rn, ruleExecInfo_t* _rei, std::list<boost::any>& _args)
	{
		try {
			...
			if ("pep_api_data_obj_put_post" == _rn) {
				...
				const char* resc_hier = getValByKey(&obj_inp->condInput, RESC_HIER_STR_KW);
				if (!resc_hier) {
				...
				if (metadata_included) {
					idx.schedule_metadata_indexing_event(object_path,
					                                     _rei->rsComm->clientUser.userName,
					                                     "a",
					                                     "v",
					                                     "u"); // "a","v","u" values are not significant; this is
				...
				auto obj_inp = boost::any_cast<dataObjInp_t*>(*it);
				object_path = obj_inp->objPath;
				const char* resc_hier = getValByKey(&obj_inp->condInput, DEST_RESC_HIER_STR_KW);
				if (!resc_hier) {
					THROW(SYS_INVALID_INPUT_PARAM, "resc hier is null");
				}
				...
				if (obj_inp->openFlags & O_WRONLY || obj_inp->openFlags & O_RDWR) {
					...
					try {
						...
						rodsLog(LOG_ERROR, "get_index_and_resource failed for [%s]", obj_inp->objPath);
					}
				}
			}
				...
				if (operation == set || operation == add) {
					if (type == collection) {
						...
						if (config->index == attribute) {
							...
							if (!avu_inp->arg3) {
								THROW(SYS_INVALID_INPUT_PARAM, "empty metadata attribute");
							}
							if (!avu_inp->arg4) {
								THROW(SYS_INVALID_INPUT_PARAM, "empty metadata value");
							}
							collection_metadata_is_new = !idx.metadata_exists_on_collection(
								object_path, avu_inp->arg3, avu_inp->arg4, NULL_PTR_GUARD(avu_inp->arg5));
						}
					}
				}
			}
				...
				const std::string rmw{
					"rmw"}; // yet to be implemented; AVU args are "like" patterns to be used in a genquery
				const std::string collection{"-C"};
				...
				if (operation == rm) {
					...
					if (type == collection) {
						...
						if (config->index == attribute) {
							...
							idx.schedule_collection_operation(irods::indexing::operation_type::purge,
							                                  logical_path,
							                                  _rei->rsComm->clientUser.userName,
							                                  value,
							                                  units);
					...
					if (type == data_object || (type == collection && config->index != attribute)) {
						...
						idx.schedule_metadata_purge_event(
							logical_path, _rei->rsComm->clientUser.userName, attribute, value, units);
					}
				}
					...
					if (type == collection) {
						...
						if (config->index == attribute) {
							...
							if (collection_metadata_is_new) {
								idx.schedule_collection_operation(irods::indexing::operation_type::index,
								                                  logical_path,
								                                  _rei->rsComm->clientUser.userName,
								                                  value,
								                                  units);
				...
				if (auto* p = getValByKey(&obj_inp->condInput, FORCE_FLAG_KW); p != 0) {
					rm_force_kw = p;
				}
			         ...
			         _rn == "pep_api_atomic_apply_metadata_operations_post")
			{
				...
				if (when == "post") {
					...
					set_symmetric_difference(pre_map.begin(),
					                         pre_map.end(),
					                         map.cbegin(),
					                         map.cend(),
					                         std::back_inserter(avus_added_or_removed));
	...
	void apply_object_policy(ruleExecInfo_t* _rei,
	...
	{
		const std::string policy_name{irods::indexing::policy::compose_policy_name(_policy_root, _indexer)};
	...
	void apply_specific_policy(ruleExecInfo_t* _rei,
	...
	{
		std::string dst;
		std::transform(src.begin(), src.end(), std::back_inserter(dst), [](wchar_t w) { return tolower(w); });
		return dst;
	}
	...
	auto get_default_mime_type(const std::string& input) -> std::string
	{
		...
		if (auto offs = input.rfind("."); offs != std::string::npos) {
			...
			try {
				...
				irods::log(
					LOG_DEBUG,
					fmt::format("In {}, function {}: Unknown extension [{}]", __FILE__, __func__, lower_case_ext));
			}
		}
	...
	void apply_metadata_policy(ruleExecInfo_t* _rei,
	                           const std::string& _policy_root,
	                           const std::string& _object_path,
	                           const std::string& _indexer,
	                           const std::string& _index_name
	                           ...
	                           const std::string& _units)
	{
		...
		args.push_back(boost::any(_object_path));
		args.push_back(
			boost::any(_attribute)); // was attr    //   -- not explicit anymore with new schema as AVU's are cataloged
		args.push_back(boost::any(_value)); // was value   //      within the indexed entry for the object itself
		args.push_back(
			boost::any(_units)); // was units   //      As this is now a no-op, we should remove these arguments.
		args.push_back(boost::any(_index_name));
